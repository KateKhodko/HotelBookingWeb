...
public class RoomTypesCommand implements Command {
	...
	public CommandResults execute(String[] params, HttpServletRequest request, HttpServletResponse response) throws ServiceException {		
    	...
    	OrderByForm orderByForm = (OrderByForm) request.getSession().getAttribute(Session.SESSION_ADMIN_ROOMTYPES_ORDER_BY);
    	OrderBy orderBy = roomTypeService.orderBy(orderByForm);
		
		int i18nId = Localization.getI18nId(request);	
		List<RoomType> roomTypes = roomTypeService.findAll(i18nId, filterForm.getArrivalDate(), filterForm.getDepartDate(), orderBy, false);			
		request.setAttribute(ROOMTYPES_ATTR, roomTypes);		
		return CommandResults.forward(Pages.ADMIN_ROOMTYPES);
	}
	...
	private void handleOrderByForm(HttpServletRequest request) {
    	...
    	if (!orderByForm.getColumnValue().isEmpty()) {
    		OrderByForm formFromSession = (OrderByForm) request.getSession().getAttribute(Session.SESSION_ADMIN_ROOMTYPES_ORDER_BY);
    		...
    		request.getSession().setAttribute(Session.SESSION_ADMIN_ROOMTYPES_ORDER_BY, orderByForm);
    	}  
    }
